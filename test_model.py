...
def test(model, queryloader, galleryloader, train_query_loader, train_gallery_loader, test_batch, loss_type,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, _, pids, camids) in enumerate(train_query_loader):
        ...
        for batch_idx, (imgs, _, pids, camids) in enumerate(train_gallery_loader):
            imgs = imgs.cuda()
...
def testPec(model,Vec, queryloader, galleryloader, train_query_loader, train_gallery_loader, test_batch, loss_type,
         euclidean_distance_loss, epoch, use_metric_cuhk03=False, ranks=[1, 5, 10, 20], return_distmat=False):
    batch_time = AverageMeter()
...
def test_PECvid(model,W,queryloader, galleryloader, train_query_loader, train_gallery_loader, test_batch,
    ...
    with torch.no_grad():
        ...
        torch.save(qf,'./cat_qf.pkl')
        torch.save(q_pids, './q_pids.pkl')
        ...
        torch.save(gf, './cat_gf.pkl')
        torch.save(g_pids, './g_pids.pkl')
        #g_paths = np.asarray(g_paths)
        # print("Extracted features for gallery set, obtained {}-by-{} matrix".format(gf.size(0), gf.size(1)))
        # print("==> BatchTime(s)/BatchSize(img): {:.3f}/{}".format(batch_time.avg, test_batch))
