...
from test_model_rerank import test_vehicleid
import numpy as np
import sys
import torch
...
from reid.utils.data.preprocessordirect import Preprocessor
...
def main(args):
    ...
    model = models.create(args.arch, num_features=args.features,
                          dropout=args.dropout, num_classes=13164)
    model=model.cuda()
    checkpoint = load_checkpoint('/home/l603a/REID/wyf/DLMB-PB/resnet18_hd_nodropout_FC_three_branch.pth.tar')
    model.load_state_dict(checkpoint['state_dict'])
    top1, map = test_vehicleid(model, query_loader, gallery_loader, query_loader, gallery_loader, 32,
                            'xent',
                            euclidean_distance_loss='xent', epoch=0,
                            use_metric_cuhk03=False, ranks=[1, 5, 10, 20], return_distmat=False)

    # torch.save(model, './resnet18_one_branch.pkl')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="CamStyle")
    # data
    parser.add_argument('-d', '--dataset', type=str, default='VehicleDirectID',
