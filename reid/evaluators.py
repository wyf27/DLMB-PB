...
class Evaluator(object):
    ...
    def queryImg(self, queryimg, query_loader, gallery_loader, query, gallery, output_feature=None, rerank=False):

        query_features, _ =extract_features(self.model, query_loader, 1, output_feature)
        gallery_features, _ = extract_features(self.model, gallery_loader, 1, output_feature)
        query_feature=collections.OrderedDict()
        for k,v in query_features.items():
            if k==queryimg+'.jpg':
                query_feature[k]=query_features.get(k)
                break
        ...
        if rerank:
            distmat = reranking(query_feature, gallery_features, querysingleimg, gallery)
        else:
            distmat = pairwise_distance(query_feature, gallery_features, querysingleimg, gallery)

            distmat = to_numpy(distmat)
            m, n = distmat.shape
            count=0
            query_ids = [pid for _, pid, _ in querysingleimg]
            gallery_ids = [pid for _, pid, _ in gallery]
